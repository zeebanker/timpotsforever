...
1. **Modular homepage** — `index.html` carries ALL CSS plus a JavaScript section loader. Content lives in:
   ...
   **Section order on the homepage** (as of May 2026): masthead → nav → jazz → hero → **whats-next** → **volunteer** → stories → newsletter → directory-events → history → give → social → footer → **Plutonian credit line**. The Plutonian credit ("Conceived, designed, developed and sponsored by a *Plutonian*") is a small ink-black band that lives in `index.html` directly, outside the loaded footer, so it appears ONLY on the homepage.
