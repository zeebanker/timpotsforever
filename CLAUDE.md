...
| **Form submissions** | Formspree (form ID `xgodbgkw`) | Two AJAX forms share this single Formspree endpoint, distinguished by a `<input type="hidden" name="_subject" value="...">` field: "Suggest a Cause" on `giving-back.html` and "Volunteer signup" on `volunteer.html`. Both post via `fetch()` with `Accept: application/json` so Formspree returns JSON instead of redirecting; on success each page swaps the form for a branded in-page thank-you state with a checkmark icon, italic "Thank *you*" / "Sign up again" copy, and a reset link. Graceful degradation: if JS is disabled, the regular form-POST takes over and Formspree handles the redirect. **Volunteer form delivery currently routes to whatever inbox is configured in Formspree for `xgodbgkw`** — owner asked for it to land in `admin@`; verify in the Formspree dashboard or create a separate form ID for volunteer-only routing. |
...
Current items, in order: **Jazz · Newsletter · Memory Lane · School History · Famous Timpots · Timpots Worldwide · Alumni Directory · Giving Back · Volunteer · Videos/Photos · Calendar**.
