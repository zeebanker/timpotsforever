...
1. **Modular homepage** — `index.html` carries ALL CSS plus a JavaScript section loader. Content lives in:
   ...
   - `sections/dynamic/` — jazz, hero, **whats-next**, **volunteer**, stories, newsletter-callout, directory-events, history, give, social
   ...
   **Section order on the homepage** (as of 2026-05-18): masthead → nav → jazz → hero → stories → newsletter → directory-events → history → give → **services** → **whats-next** → **volunteer** → social → footer → **Plutonian credit line**. Three community-action bands (whats-next, volunteer, plus the new Timpots @ Work services band) were moved from the top of the homepage to a cluster at the bottom, just above social. Sequence reads as a build-up: Giving Back · Timpots @ Work · Where we're headed · It takes a village · Be social. The Plutonian credit ("Conceived, designed, developed and sponsored by a *Plutonian*") is a small ink-black band that lives in `index.html` directly, outside the loaded footer, so it appears ONLY on the homepage.
...
| **Form submissions** | Formspree (form ID `xgodbgkw`) | Three AJAX forms share this single Formspree endpoint, distinguished by a `<input type="hidden" name="_subject" value="...">` field: "Suggest a Cause" on `giving-back.html`, "Volunteer signup" on `volunteer.html`, and "Timpots @ Work listing" on `services.html`. Each posts via `fetch()` with `Accept: application/json` so Formspree returns JSON instead of redirecting; on success each page swaps the form for a branded in-page thank-you state with a checkmark icon, italic "Thank *you*" copy, and a reset link. Graceful degradation: if JS is disabled, the regular form-POST takes over and Formspree handles the redirect. **All three subjects route to the same Formspree inbox** — triage by `_subject` line; consider creating dedicated form IDs per use if volume warrants. |
| **Map data** | `public_alumni_map` view in Supabase | Anonymous SELECT; powers `map.html`. No login required. |
| **Site analytics** | Cloudflare Web Analytics (free tier) | Beacon snippet (`https://static.cloudflareinsights.com/beacon.min.js`) injected just before `</body>` on every HTML page in the repo root. Token `48e25714d319472e922b02e76638ea87`. **No cookies, no PII, no consent banner needed** — anonymous, aggregated stats only (page views, top pages, country, browser/OS, referrers, Core Web Vitals). Dashboard: Cloudflare dashboard → Analytics & Logs → Web Analytics → `timpotsforever.org`. Disclosure paragraph lives in `privacy.html` Section 8 ("Cookies and Tracking Technologies") subsection B. |
...
| `volunteer.html` | public | Volunteer signup page. Hero ("It takes a *village*") + warm-collective intro card + grid of 6 role cards (Fact-check · Photo archive · Web team · Newsletter · Ambassador · Giving-back) each with stroke SVG icon + Formspree form (name, email, role-interest checkboxes for all six, optional textarea). Companion homepage band lives at `sections/dynamic/volunteer.html` ("LEND A HAND" chip + "It takes a *village*" headline + maroon line-drawing of stacked hands on a white-with-gold-border card). |
| `services.html` | public | **Timpots @ Work** — alumni-business directory submission page. Hero + intro card (Timpots-only, free, references-may-be-required language) + 3-step "How it works" + Formspree submission form (name, email, batch, business name, 11-option category dropdown, one-line tagline, longer description, website, phone, full business address with street/apt/city/state/postal/country, **Reference 1 & 2** sub-blocks each with name+phone+email, optional notes) + fine-print card ("What this is, and isn't") + **Ground rules** punchy summary card (white bg, garnet left-border, 6 gold-checkmark bullets, link to full T&C Section 14) + footer. AJAX → in-page thank-you swap. Form posts to Formspree `xgodbgkw` with `_subject: "Timpots @ Work listing — Timpots Forever"`. Companion homepage band at `sections/dynamic/services.html`. |

## Navigation (`sections/static/nav.html`)

Current items, in order: **Jazz · Memory Lane · School History · Famous Timpots · Timpots Worldwide · Alumni Directory · Giving Back · Timpots @ Work · Volunteer · Videos/Photos · Calendar**.
...
- **Newsletter** — removed from nav, footer Explore column, and homepage callout "Get notified →" CTA in 2026-05-18 update (the page `newsletter.html` still exists and is reachable by direct URL; the homepage callout copy now reads "Under development" instead of "Coming July 2026"). To restore, re-add `<a href="newsletter.html">Newsletter</a>` between Jazz and Memory Lane in nav, between Jazz and Memory Lane in footer Explore column, and restore the `<a href="newsletter.html" class="nc-btn">Get notified →</a>` in `sections/dynamic/newsletter-callout.html`.

Items added:
- **Timpots @ Work** — alumni-business directory; added between Giving Back and Volunteer (2026-05-18). Footer link in the second Explore sub-column.
...
| `services.html` | public | ✅ Premium | **Timpots @ Work** — alumni-business directory submission page. Hero + intro (Timpots-only + references-may-be-required language) + 3-step explainer + full Formspree form with category dropdown, full address, two Reference sub-blocks + "What this is, and isn't" fine-print card + **Ground rules** punchy summary card (white bg, garnet left-border, gold-checkmark bullets, link to T&C #services-rules). |
...
- **Recolorable line-art pipeline** (used by stories cards, the volunteer band, and the school-history crests): take a JPEG/PNG line drawing with white background, run it through PIL to threshold to crisp black-on-transparent, dilate the alpha with `ImageFilter.MaxFilter(5)` for slightly thicker lines, save as PNG. Reference the PNG (or SVG for the crests) as a CSS `mask-image` on a `<div class="card-art …">` and color via `background-color`. This gives crisp recolorable line art that scales without pixelation. Existing examples in `images/`: `teamwork-line.png` (Where Are They Now card), `classroom-line.png` (Class Notes), `teacher-line.png` (Teachers We Remember), `hands-line.png` (Volunteer band), `crest-old.svg` + `crest-new.svg` (school-history crest-callout).
...
- ✅ **Cloudflare Web Analytics wired up (2026-05-18)** — privacy-first traffic analytics added across the site. Beacon snippet (`<script defer src='https://static.cloudflareinsights.com/beacon.min.js' data-cf-beacon='{"token": "48e25714d319472e922b02e76638ea87"}'></script>`) injected just before `</body>` on all 18 HTML pages (index + 17 standalone). No cookies, no PII, no consent banner. Privacy disclosure paragraph rewritten in `privacy.html` Section 8.B to explain what Cloudflare Web Analytics collects (anonymous aggregate stats) and link to Cloudflare's privacy commitments. To view stats: Cloudflare dashboard → Analytics & Logs → Web Analytics → `timpotsforever.org`. Data starts flowing 1–2 hours after first page load with the snippet live.
- ✅ **Homepage section order reshuffled (2026-05-18)** — moved the three community-action bands (whats-next, volunteer, and the new services band) from interspersed positions near the top of the homepage into a coherent cluster at the bottom, just above social. New order: masthead · nav · jazz · hero · stories · newsletter · directory-events · history · give · **services** · **whats-next** · **volunteer** · social · footer. Sequence reads as a build-up: Giving Back · Timpots @ Work · Where we're headed · It takes a village · Be social. Both the `<div id="section-*">` placeholders in `index.html` and the loader registry order were updated to match (loader order is cosmetic-only — actual display order depends on placeholder order, but kept aligned for sanity).
- ✅ **Timpots @ Work shipped (2026-05-18)** — new alumni-business directory feature. **New files**: `services.html` (full standalone page following volunteer.html pattern: canonical `.members-header`, hero, intro card with Timpots-only + references-may-be-required language, 3-step "How it works" explainer, full Formspree submission form, "What this is, and isn't" fine-print card, and a punchy "Ground rules" summary card with garnet left-border + gold checkmark bullets linking to T&C `#services-rules`); `sections/dynamic/services.html` (homepage band — gold top border, briefcase line-icon, "Promote what you *do*" headline, "Add your listing" CTA; currently sits between Giving Back and Where We're Headed in the community-action cluster at the bottom of the homepage). **Form fields**: name, email, year/batch at Timpany (for verification), business/service name, 11-option category dropdown (Professional services · Healthcare & wellness · Technology & engineering · Creative & design · Education & coaching · Hospitality & food · Retail & goods · Real estate · Finance & investing · Trades & home services · Other), one-line tagline (max 140 chars), longer description (max 600), website URL (optional), phone/WhatsApp (optional), full business address (street, apt/suite, city, state/province/region, postal/ZIP, country — only city + country required), two **Reference sub-blocks** (each with name + phone + email, optional but encouraged), free-form "anything else." Form posts to existing Formspree `xgodbgkw` with hidden `_subject: "Timpots @ Work listing — Timpots Forever"`. **Touchpoints**: nav link in `sections/static/nav.html` between Giving Back and Volunteer; footer Explore-column link in `sections/static/footer.html` between Giving Back and Videos/Photos; `index.html` section loader registry entry, `<div id="section-services"></div>` placeholder (currently between `section-give` and `section-whats-next`), and new `.feature-band.services` + `.services-mark` CSS.
- ✅ **Terms & Conditions expanded to 26 sections (2026-05-18)** — four new sections added: **§3 Non-Commercial Purpose** (memorial framing + never-solicit-funds promise + impersonation alert directing reports to `admin@`); **§13 No Unsolicited Marketing or Solicitation** (site-wide ban on using directory/listing/profile data for spam, scraping, MLM recruiting, etc., with explicit reference to CAN-SPAM/GDPR/India IT Act + "any other applicable regulations"); **§14 Timpots @ Work Listings** (8 sub-rules a–h: eligibility, accuracy, prohibited listings list, reference-data handling promise, review discretion, no endorsement, updates/removal, reporting concerns — anchored at `#services-rules` for direct linking); **§17 No Contract; No Agency** (use of site doesn't create any formal relationship); **§18 No Promises or Warranties** ("as is/as available" disclaimer + 7-bullet list of things specifically NOT guaranteed, including "coming soon" features being delivered, finding any particular alumnus, quality of any Timpots @ Work listing). Existing sections 3–21 shifted to 4–22, then again to 4–26 after the additions. Section 3 anchored at `#non-commercial` for the services.html fine-print cross-link.
- ✅ **Site temporarily parked then restored (2026-05-17)** — for ~24 hours `timpotsforever.org` served a maroon "Timpots Forever — coming soon!" page from a new `coming-soon` branch (minimal `index.html` only; full site files removed on that branch). Cloudflare Workers project's Branch Control was flipped Production branch to `coming-soon`, then back to `main` next day. Cloudflare lesson: this project uses the **Versions & Deployments** model — `npx wrangler versions upload` only creates a new version; production traffic doesn't switch until the version is **Promoted** from the Version History row's per-row Promote button (the dashboard's "New Deployment" button is a separate file-uploader action, not a version-promote). Deploy command now `npx wrangler deploy` so future pushes auto-promote. `coming-soon` branch is preserved on origin for fast re-parking.
- ✅ **Seek Truth logo partially restored (2026-05-18)** — two of the four logo locations removed on 2026-05-17 were brought back: (B) `.hero-bg::after` watermark behind the homepage hero (340×340, opacity 0.22, right-aligned) and (C) `.feature-band::before, .give-section::before` watermarks behind every feature-band and give-section (380×380, opacity 0.07, right-of-center). The `--logo-watermark` CSS variable (~108 KB base64 PNG data URI on `index.html` line 22) and `.feature-band > *, .give-section > *` z-index normalization rule were restored to support them. **NOT restored** (per owner's selection): (A) masthead top-left `.brand-logo` and (D) `.site-footer::before` footer watermark. The `.site-footer` is no longer in the combined watermark selector — added only feature-band + give-section.
- ✅ **Homepage micro-edits (2026-05-18)** — Newsletter link removed from nav and footer Explore column (page still reachable via direct URL only); homepage "Newsletter" callout band updated: "Coming July 2026" → "Under development" label and "Get notified →" button removed entirely. "PHILANTHROPY · FEATURED CAUSE" label on `sections/dynamic/give.html` reworded to "GIVING BACK · FEATURED CAUSE". "Reunions" link removed from footer Support column (calendar.html still linked from nav).
...
- ✅ **School history page — autobiography-led rewrite (2026-05-16)** — Owner instruction: take Dr. Mercy Jeyaraja Rao's autobiography *My Story* (Nurturing Faith / Good Faith Media, ISBN 978-1-63528-136-1) as authoritative for school history from 1954 onward, NOT alumni group-chat recollections. Hero re-framed to "Founded by Rev. A.V. Timpany (1840–1885) in Kakinada around 1880 and relocated to Visakhapatnam in 1931 — what began as the Anglo-Indian Protestant Middle School next to St. John's Church grew, under Dr. Mercy Jeyaraja Rao's forty-eight-year stewardship…". Timeline rebuilt from primary sources: c.1880 founding · 1882 Kakinada building (still standing, houses A.V. Timpany Memorial Library) · 1885 death · 1931 Vizag relocation · 1 Jul 1954 Jazz joins as teacher of grades 4–5 (Rs.120/$24, 70 students, Miss Hellyer recruits her) · 1 Jan 1959 first Indian Headmistress · 1964 Center for Special Education · 1964–65 Waltair Uplands move · etc. Five autobiography-led story sections replace the alumni-led ones: "The Anglo-Indian Protestant Middle School," "From Old Town to Waltair Uplands," "Leadership," "A Co-Educational School by Choice," "Beyond the Standard Curriculum." Direct quotes from *My Story* are rendered in a new `.autobio-quote` blockquote style (gold left border + italic Playfair + small-caps `<cite>` attribution). Photo collage of the original Kakinada building added at `images/kakinada-original-school.jpeg` with figcaption "An early Timpany School building built in 1882 in Kakinada. It was later moved to Visakhapatnam in 1931." Source PDF lives at `~/Downloads/Timpany history Jazz's autobiography .pdf` (curly apostrophe in filename); back cover photo for Jazz's bio at `~/Downloads/back-cover.jpeg` (or similar).
- ✅ **Jazz bio rewrite on `jazz.html` (2026-05-16)** — Bio paragraphs reworked to incorporate the back-cover autobiography facts: birth 4 Jul 1930 in Vizag (fourth daughter of Mary and Daniel Jeyaraja Rao); 1947 conversion to Christianity at age 17; education (BA local · BT Lady Willingdon, Chennai · Honorary D.Div. Judson College, Marion AL · D.Min. Asian Institute of Theology, Bangalore); four roles at Timpany (Headmistress, Principal, Executive Director, General Director); retirement-scale numbers (3 campuses · >3,000 students · KG–12 · alumni in 40+ countries); ministry work (International Women's Ministry 1960–2000, Asian Baptist Women's Union VP/President, BWA Men's & Women's Departments); US citizenship 3 Oct 2013 with Sandy Wisener's help. Also corrected the previous bio's "single small room with ten students" — the autobiography clearly says 70 students in the Sunday-School building next to St. John's Church.
- ✅ **Crest-callout switched to SVG masks (2026-05-16)** — Both `images/crest-old.jpg` and `images/crest-new.png` were replaced with vector tracings `images/crest-old.svg` (76 KB, portrait shield, 638×869) and `images/crest-new.svg` (40 KB, landscape book + flame + waves, 720×361). 75% smaller combined, infinitely scalable. `school-history.html`'s `.crest-callout` now renders the crests in **bright-gold via CSS `mask-image` on a deep-maroon box** (`.crest-callout` background = `var(--brand)`, `.crest-art` background-color = `var(--gold-bright)` with mask-image set to the SVG, captions in gold-bright, body text in white) — gold-on-maroon matches the school's brand. The original raster crest files are no longer referenced anywhere; keep or delete at owner's discretion.
- ✅ **Stories cards re-themed with recolorable line-art (2026-05-16)** — All three story cards on the homepage now use the **CSS-mask line-art pattern** instead of inline SVGs/silhouettes. (1) "Where Are They Now" (c1, maroon gradient) shows `teamwork-line.png` (4-vignette continuous line drawing: gear/team → handshake → growth-arrow → flag) in gold-bright. (2) "Class Notes" (c2 — bg changed from gold-gradient to maroon gradient matching c1) shows `classroom-line.png` (teacher + 4 students at desks) in gold-bright. (3) "The Teachers We Remember" (c3, leather gradient — preserved for visual variety) shows `teacher-line.png` (teacher at blackboard) in gold-bright. All three rendered via `<div class="card-art teamwork|classroom|teacher" role="img" aria-label="…">` with CSS mask-image and `background-color: var(--gold-bright)`. PNGs are generated by the PIL pipeline described in **Conventions** above (threshold + `MaxFilter(5)` dilation + transparent bg).
- ✅ **Volunteer page + homepage band shipped (2026-05-16)** — New `volunteer.html` standalone page with "It takes a *village*" hero, intro card framing the three working groups, grid of 6 role cards (each with stroke SVG icon), and Formspree signup form (name, email, 6 role-interest checkboxes, optional textarea). Form posts to the existing Formspree form `xgodbgkw` with a hidden `_subject: "Volunteer signup — Timpots Forever"` field to distinguish from "Suggest a Cause" submissions; same AJAX pattern (in-page thank-you swap, no Formspree redirect). Companion `sections/dynamic/volunteer.html` homepage band lives between hero and stories — cream background, maroon top border, "LEND A HAND" chip, "It takes a *village*" headline, short pitch, "SEE HOW TO HELP →" CTA, right column is a white-with-gold-border card containing `hands-line.png` (stacked-hands continuous line drawing) recolored in maroon via CSS mask. "Volunteer" added to nav (between "Giving Back" and "Videos/Photos") and to the footer Support column.
- ✅ **Hero rewrite + "Where We're Headed" band (2026-05-16)** — The hero ("Out of a memorial, a *movement*") now runs as **three narrative paragraphs** describing (1) Jazz's passing on 22 Mar 2026, the Vizag memorial organized by Meenakshi Anantram and other alumni on Timpany's grounds, the late-March Dallas funeral attended by several alumni, livestreams to the rest of the world; (2) the WhatsApp group that grew past **600 Timpots in weeks**, hundreds of tributes, old photos surfacing from the 60s–80s, former teachers joining, songs across time zones; (3) three alumni groups self-organizing (website · *Timpany Times* newsletter · memorial events & giving-back) — "Jazz's work, continued — and only just begun." The hero's `.hero-dek` is now a `<div>` wrapping multiple `<p>` tags; CSS handles inter-paragraph spacing. `.hero` height changed from fixed `560px` to `min-height: 560px` so the longer content doesn't get clipped. A new `sections/dynamic/whats-next.html` band sits between the hero and Volunteer: "WHERE WE'RE HEADED" chip + "And we're just *getting started*" headline + 2-column bulleted list of 8 upcoming features (Jeyaraja Rao Memorial Debate Competition · Spotlights · Career Corner · Alumni Directory · Professional Services Corner · Social channels · Vlogs & podcasts · "more as Timpots tell us") + italic handoff line "None of this happens without you" that leads straight into the Volunteer CTA. New `.feature-band.whats-next` CSS variant in `index.html` (white bg, gold top border, gold dot-bullets, brand red strong-text).
- ✅ **Hero portrait card re-themed as twin brand marks (2026-05-16)** — The right-side floating maroon card that used to hold a silhouette + "Featured Alumna" label now shows **two equal-weight wordmarks side-by-side vertically**: globe icon + "Timpots *Forever*" (the website) and newspaper icon + "The *Timpany Times*" (the newsletter), separated by a thin gold rule. Both wordmarks at 26px Playfair, identical sizing, italic gold-bright second word. Stroke SVG icons (1.6px stroke width) match the rest of the site's icon language. The old `.hero-mark-emblem` (Seek Truth mask) was removed.
- ✅ **Plutonian credit line on landing page only (2026-05-16)** — A small ink-black band at the very bottom of `index.html`, OUTSIDE the loaded footer, reads: "Conceived, designed, developed and sponsored by a *Plutonian*." Because it lives in `index.html` directly (after `<div id="section-footer"></div>`), it only shows on the homepage — standalone pages don't have it.
- ✅ **Newsletter callout tightened against the stories row above (2026-05-16)** — The newsletter section is wrapped in `<section class="stories-row">` like the stories grid, so it inherited the row's `padding-top: 60px` on top of the stories row's `padding-bottom: 70px` — producing ~130–180px of dead whitespace below "More stories coming soon." Added a `.newsletter-section` modifier class (`<section class="stories-row newsletter-section">`) with `padding-top: 0; padding-bottom: 80px;` to tighten the gap.
- ✅ **Famous Timpots roster edits (2026-05-16)** — Removed Shobita Dhulipala from the Arts & Entertainment section; added Sonika Mahajan (Chopra) — Mumbai, Jupiter house, Bollywood actress and model — in her place.
- ✅ **Mobile nav drawer fix (2026-05-16)** — With 11 nav items (after adding "Volunteer"), the mobile drawer was clipping the last two ("Videos/Photos" + "Calendar") on shorter viewports because of `max-height: 80vh` + `overflow: hidden`. Fix: tightened item vertical padding from 16px → 12px so all 11 items fit naturally on most modern phones, AND added `overflow-y: auto` + `-webkit-overflow-scrolling: touch` + raised cap to 85vh on `body.menu-open .main-nav` so the drawer scrolls gracefully when needed (older phones, landscape, future items).
- ✅ **Seek Truth logo scrubbed from the homepage (2026-05-16)** — Per owner instruction, the Seek Truth emblem no longer appears in (1) the masthead top-left (`<span class="brand-logo">` removed from `sections/static/masthead.html`; replaced with an empty `<div class="masthead-left">` placeholder so the 3-col grid still centers the "Timpots Forever" wordmark), (2) the ghosted watermark behind every `.feature-band` / `.give-section` (the `::before` pseudo-element rule deleted), or (3) the ghosted watermark behind the footer (`.site-footer::before` rule deleted). The `--logo-watermark` CSS variable (a ~108 KB base64 data URI on line 22 of `index.html`) and the `.brand-logo` mask rule were also removed since they're no longer referenced. **The emblem still lives at `images/logo.png`** and is used by `map.html` (`L.imageOverlay` pinned over Visakhapatnam with the garnet `feColorMatrix` tint) — leave that file in place.
- ✅ **Disclaimer copy unified across all 15 footers (2026-05-16)** — The shared homepage footer (`sections/static/footer.html`) and the 14 standalone-page inline footers (accessibility, calendar, directory, famous-timpots, giving-back, jazz, memory-lane, newsletter, privacy, profile, school-history, terms, videos-photos, volunteer) now all use the same disclaimer line: *"Timpots Forever is an independent alumni and friends community initiative and is not affiliated with, sponsored by, endorsed by, or officially connected with Timpany School or its management. References to Timpany School are historical and descriptive only."* The longer legal paragraph in `terms.html` body (around line 255) was intentionally left untouched — that's the in-page legal text, not the footer line.
